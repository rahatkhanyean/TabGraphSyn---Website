...
def evaluation_func(fake_file_ls, dataset_name = "Nothing", train_name = None):
    ...
    for fake_file_name in fake_file_ls:
        ...
        df = df_kept.copy()
        fake_df = fake_df_kept.copy()
        fake_df = fake_df.reindex(columns=df.columns)
        for col in df.columns:
            if df[col].dtype.kind in 'biufc':
                df[col] = pd.to_numeric(df[col], errors='coerce')
                fake_df[col] = pd.to_numeric(fake_df[col], errors='coerce')
                mean_val = df[col].mean()
                df[col] = df[col].fillna(mean_val)
                fake_df[col] = fake_df[col].fillna(mean_val)
            else:
                df[col] = df[col].fillna('missing')
                fake_df[col] = fake_df[col].fillna('missing')
        ...
        if len(numeric_cols) > 0:
            concat_df[numeric_cols] = concat_df[numeric_cols].fillna(concat_df[numeric_cols].mean())
        if len(categorical_columns) > 0:
            concat_df[categorical_columns] = concat_df[categorical_columns].fillna('missing')
