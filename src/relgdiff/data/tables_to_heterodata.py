...
def tables_to_heterodata(
    ...
    for relationship in metadata.relationships:
        ...
        data[parent_table, foreign_key, child_table].edge_index = torch.tensor(
            fks.values.T, dtype=torch.long
        )
        data[child_table, foreign_key, parent_table].edge_index = torch.tensor(
            fks.loc[:, [child_primary_key, foreign_key]].values.T, dtype=torch.long
        )
    ...
    for key in metadata.get_tables():
        ...
        if key in latents:
            data[key].y = torch.tensor(latents[key][:, 1:, :], dtype=torch.float32).reshape(
        ...
        if pos_enc is not None and key in pos_enc:
            ...
            order = torch.tensor(values / np.timedelta64(1, "D"), dtype=torch.long).long()
            data[key].pe = order

    # set positional encoding to data object
    if pos_enc is not None:
        data.pe_dict = pos_enc
